...
import (
	...
	"github.com/ericsage/cxconverter/cxpb"
...
func transfer(uploader *s3manager.Uploader, bucketName string, name string, id string, index int) {
...
func convert(stream io.Reader) {
	cx := &cxpb.CX{}
	var json []map[string][]interface{}
	dec := json.NewDecoder(strea)
	dec.Decode(&json)
	for _, aspect := range json {
		switch(aspect) {
		case "nodes":
			for _, node := range :q
...
func main() {
	...
	//	transfer(uploader, bucketName, network.Name, network.ID, index)
	//}
}
